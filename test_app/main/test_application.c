...
{
  // Deinitialize tasks in reverse order
  // esp_err_t ret = state_machine_task_deinit();
  // if (ret != ESP_OK)
  // {
  //   ESP_LOGE(TAG, "Failed to deinitialize State Machine task");
  //   return ret;
  // }

  esp_err_t ret = aws_iot_task_deinit();
  if (ret != ESP_OK)
  {
    ESP_LOGE(TAG, "Failed to deinitialize AWS IoT task");
    return ret;
  }

  ret = gpio_task_deinit();
  if (ret != ESP_OK)
  {
    ESP_LOGE(TAG, "Failed to deinitialize GPIO task");
    return ret;
  }

  ret = wifi_task_deinit();
  if (ret != ESP_OK)
  {
    ESP_LOGE(TAG, "Failed to deinitialize WiFi task");
    return ret;
  }

  ESP_LOGI(TAG, "Application deinitialized");
  return ret;
}